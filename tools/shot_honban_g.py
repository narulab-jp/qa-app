# -*- coding: utf-8 -*-
"""冊G（地域調査）の問題が、実際の画面でどう見えるかを撮る（目視確認用）。"""
import base64
import io
import os
import shutil
...
TARGETS = [1, 2, 4, 6]      # 変化の把握・変化と地形・仮説の検証・言いすぎ
...
def main():
    ...
    if free:
        ...
        time.sleep(1.5)
    ud = os.path.join(os.environ["TEMP"], "edge_shot_g")
    ...
    try:
        ...
        for no in TARGETS:
            c.ev("(function(){"
                 ...
                 "document.getElementById('unit-G').click();"
            ...
            fp = os.path.join(OUT, "G_%02d.png" % no)
            io.open(fp, "wb").write(base64.b64decode(d["result"]["data"]))
            print("  冊G 問%d → %s" % (no, fp))
    finally:
        try:
            p.kill()
        except Exception:
