...
# 地図記号の中の字（文・〒・介など）の下限。紙に出たときの大きさ。
# 1.62mm ＝ 約4.6pt。これを下回ると何の記号か読めない。
MM_GLYPH = 1.62
...
def _glyph(x, y, ch, r=7.0, size=10.0):
    """丸の中に字を入れる地図記号。
      ...
      小さく置かれる図では 10単位＝1.30mm（3.7pt）まで小さくなり、
      何の記号か読めなくなっていた。下限を割るときは、字と丸を
      同じ倍率で大きくする（丸だけ据え置くと字がはみ出す）。
    """
    k = max(1.0, (MM_GLYPH / _UNIT_MM[0]) / size)
    size *= k
    r *= k
