...
def factory_map():
    ...
    b.append(txt(596, 396, "海", 14, "end", "bold"))
    ...
    for (x, y) in [(120, 180), (260, 200), (520, 180)]:
        b.append(poly([(x, y - 7), (x + 7, y + 5), (x - 7, y + 5)], 1.5,
                      fill="hatch:diag", close=True))
    # 原料を運びこむ港（工場の並びと同じ海ぞい）
    for (x, y) in [(185, 292), (365, 294), (535, 296)]:
        b.append(rect(x - 7, y - 7, 14, 14, "#000", 1.0))
    # 人口の多い都市（内陸に置く。市場に引かれているかを見分けるため）
    for (x, y) in [(215, 145), (470, 195)]:
        b.append(circle(x, y, 9, "#fff", 1.6))
        b.append(circle(x, y, 4, "#000", 1.0))
    for (nm, x, y) in [("ア", 230, 240), ("イ", 300, 175), ("ウ", 560, 240),
                       ("エ", 130, 150)]:
        b.append(circle(x, y, 11, "#fff", 1.6))
        b.append(txt(x, y + 5, nm, 13, "middle", "bold"))
    ly = 436.0
    ...
    # 凡例は区分の名まえだけ。「臨海部の」のような説明は入れない。
    b.append(sym_factory(84, ly + 40))
    b.append(txt(98, ly + 44, "工場", 11))
    b.append(circle(170, ly + 40, 5, "#000", 1.0))
    b.append(txt(182, ly + 44, "炭田", 11))
    b.append(poly([(250, ly + 33), (257, ly + 45), (243, ly + 45)], 1.5,
                  fill="hatch:diag", close=True))
    b.append(txt(266, ly + 44, "鉄鉱石の産地", 11))
    b.append(rect(374, ly + 33, 12, 12, "#000", 1.0))
    b.append(txt(392, ly + 44, "港", 11))
    b.append(circle(440, ly + 40, 8, "#fff", 1.6))
    b.append(circle(440, ly + 40, 3.5, "#000", 1.0))
    b.append(txt(454, ly + 44, "人口50万人以上の都市", 11))
    b.append(txt(20, H - 12, NOTE_FAKE, 11))
    return svg(W, H, "\n".join(b), "工業の分布")
...
FIGURES = {
    ...
    # 「地形のようす」の列は外した。設問の答えがそのまま書いてあり、
    # 表の1列を写すだけで解けてしまっていたため。
    # 地形は、資料1（プレート境界と火山・地震の分布）から読ませる。
    ...
    # --- 資源・産業 ---
    # 「電力のようす」の列は外した。設問の答え（大気汚染が問題／
    # 再生可能エネルギーの割合が高い など）がそのまま書いてあり、
    # 帯グラフを見なくても解けてしまっていたため。
    # 代わりに、判断の材料になる数値（大気汚染の指標と輸出入）を置く。
    "CR2_table.svg": lambda: table(
        "資料2　架空の4か国のようす",
        ["国", "一人当たりGNI\n（ドル）", "自国で産出する\nおもな資源",
         "エネルギーの\n輸入依存度（％）", "都市部の大気中の\n微粒子の量（指数）"],
        [["カ国", "9,800", "石炭", "8", "184"],
         ["キ国", "34,000", "天然ガス", "12", "62"],
         ["ク国", "46,000", "ほとんどない", "91", "58"],
         ["ケ国", "58,000", "水力・地熱", "24", "31"]],
        [64, 130, 132, 130, 148]),
